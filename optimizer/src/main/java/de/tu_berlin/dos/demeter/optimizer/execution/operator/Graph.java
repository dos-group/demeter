package de.tu_berlin.dos.demeter.optimizer.execution.operator;
...
public enum Graph implements SequenceFSM<Context, Graph> {
    ...
    INITIALIZE {
        ...
        public Graph runStage(Context ctx) throws Exception {
            ...
            LOG.info(ctx.job.deploymentWithAutoscaler());
            ctx.cm.createJob(ctx.job.deploymentWithAutoscaler(), ctx.job.nodePort(), ctx.job.jobName, ctx.job.port);
            ctx.et.start(60);
